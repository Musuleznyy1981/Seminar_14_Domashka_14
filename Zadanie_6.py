# Задание №6
# Проверить, что выкидываются правильные исключения (файл Exceptions.py) для кода Semin_13Zadanie5
...
from Seminar_14.Semin13_Zadanie5 import UserWorkshop, User
...
def test_access_fail_1(set_up):                               # тест на проверку того, что  будет выброс исключения AccesErorr при вводе невалидных данных, AccesErorr  прописан в Exceptions.py
    ...
    with pytest.raises(AccesErorr, match='Access denied'):
        set_up.login('Musuleznovs', '1')                      # передаются невалидные данные


def test_access(set_up):                                    # тест на проверку валидных данных, то что должна вернуться
    assert set_up.login('Musuleznyy', '1') == '5'             # вводятся валидные двнные и проверяем на совпадение с результатом
